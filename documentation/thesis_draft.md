...
#### 2.1.1 Selection of Faster-Whisper
...
#### 2.2.1 Optimization with CTranslate2
...
#### 2.3.1 Dual-Engine TTS Architecture
...
#### 2.4.1 VAD Implementation Strategy
...
#### 2.5.1 Communication Protocol Selection
...
#### 3.1.1 Architectural Design Decisions
...
│       ▼                                                         │
│  ┌──────────┐                                                   │
│  │ Meet/Zoom│ ◄── Conference app uses BlackHole as mic input    │
│  └──────────┘                                                   │
└─────────────────────────────────────────────────────────────────┘
...
- **Web Audio API (Frontend Audio Processing):**
  - **Justification:** Provides powerful, low-level control over audio processing directly in the browser. It enables efficient microphone capture, real-time manipulation (e.g., resampling, chunking via Web Audio Worklets), and playback of synthesized audio without relying on server-side processing for these client-specific tasks. This is crucial for minimizing frontend-induced latency.
...
- **WebSockets (Real-time Communication):**
  - **Justification:** Essential for the bidirectional, low-latency streaming of audio and text data between the frontend and backend. As detailed in Section 2.5.1, WebSockets significantly outperform traditional HTTP for continuous real-time communication by reducing overhead.
...
- **Chart.js (Latency Visualization):**
  - **Justification:** A lightweight, flexible JavaScript charting library that is easy to integrate into a plain HTML/CSS/JS frontend. It allows for clear and interactive visualization of real-time latency metrics, which is a key objective for evaluating the system's performance.
...
- **Python (Backend Language):**
  - **Justification:** Python is the de facto language for AI/ML development, with a rich ecosystem of libraries (PyTorch, NumPy, Hugging Face, CTranslate2, Piper, Coqui TTS) that are directly used by the backend models. Its readability and extensive community support accelerate development.
...
- **Apple Silicon (M1/M2/M3) (Target Hardware):**
  - **Justification:** The development environment leverages Apple Silicon for its high performance-per-watt, integrated GPU (MPS), and unified memory architecture. This provides a powerful local development and testing platform for AI models, allowing for efficient iteration and optimization.
...
- **`ffmpeg` (Audio Processing Utility):**
  - **Justification:** A powerful, industry-standard command-line tool for handling multimedia data. It is used for robust audio format conversions (e.g., WebM to WAV) and ensuring consistent audio properties (sample rate, channels) before processing by AI models.
...
- **`openssl` (SSL Certificates):**
  - **Justification:** Used for generating self-signed SSL certificates, enabling HTTPS/WSS communication for secure local development and testing. This is a standard practice for securing web applications.
...
- **MPS Instability:** Initial tests on Apple Silicon's MPS backend revealed stability issues (crashes).
- **Mitigation:** As detailed in Section 5.2.6, the system was configured to force CPU execution for XTTS v2, prioritizing stability over raw inference speed.

**Trade-off:** While CPU execution results in higher latency (RTF ~1.72) compared to theoretical GPU performance, the stability and consistent quality improvements justified this trade-off for the bachelor's thesis demonstration.
...
#### 5.2.5 Concurrent Performance Testing and Scalability Analysis

A critical requirement for a real-time speech translation system in conferencing environments is the ability to handle multiple simultaneous users. This section presents the results of comprehensive concurrent performance testing designed to evaluate the system's scalability and identify capacity limits.

##### 5.2.5.1 Test Methodology

**Objective:** Validate the system's capacity to handle multiple concurrent users performing real-time speech translation simultaneously, measure performance degradation under load, and identify scalability bottlenecks.
...
- **Test Script:** Custom Python script (`test/concurrent_performance_test.py`) using WebSocket connections to simulate real users

**Metrics Collected:**

- **Session Success Rate:** Percentage of sessions that completed successfully
- **End-to-End Latency:** Time from audio input start to TTS output completion
- **Component Latencies:** Individual STT, MT, and TTS processing times
- **Resource Usage:** CPU and memory consumption during concurrent load
- **Timeline Data:** Precise timestamps for all events (audio sent, transcription, translation, TTS)
...
| **TTS Time**    | 0.27s   | 0.05s | 0.67s  | 0.19s   |

**Resource Usage:**

- CPU: Average 45%, Peak 68%
- Memory: Average 520MB, Peak 780MB

**Analysis:**

- All 5 sessions completed successfully with no failures
- E2E latency variability (4.45s - 27.40s) correlates with audio file length:
  - Short files (7 words): ~4-6s E2E latency
  - Long files (65 words, 8 segments): ~27s E2E latency
- System remained stable with CPU below 70%
- True concurrent processing verified via timeline overlap
- For typical utterances (7-15 words), latency averaged 4.5-6s, meeting conversational requirements
...
- [1] OpenAI. (2022). _Whisper_. Retrieved from [https://openai.com/research/whisper](https://openai.com/research/whisper)
