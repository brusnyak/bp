...
- [ ] T026 [US5] Record ~10-60 min of reference audio in EN (and SK, once available). NOT DONE - still needs real recording; only the existing ~0.6 min of test clips exist, confirmed too short (script now warns below 10 min). Note: could not find an authoritative source for the exact minimum - the 10-20 min figure is a reasoned estimate carried from earlier research, not a hard citation. Treat as a planning floor to re-validate empirically, not a proven number.
...
- [ ] T028 [US5] NOT DONE - three-tier selection logic in `backend/main.py`/voice-selection path not yet wired. Deliberately left for after T026 (real data) and a real T029 quality pass, not blocked on tooling.
- [~] T029 [US5] Latency delta MEASURED: fine-tuned Piper RTF 0.0295 vs XTTS RTF 1.72 (`documentation/COQUI_TTS_PERFORMANCE_REPORT.md`) - ~58x. Voice-similarity comparison NOT done (toy 3-step model isn't representative) - re-run once T026/a real training pass exists.
